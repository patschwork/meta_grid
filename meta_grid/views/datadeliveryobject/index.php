	...
	?>
        ...
        'columns' => [
            ...
            [
             ...
             'value' => function($model) {
             		return $model->fk_tool_id == "" ? $model->fk_tool_id : (isset($_GET["searchShow"]) ? $model->fkTool->tool_name . ' [' . $model->fk_tool_id . ']' : ($model->fkTool=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkTool', 'this_id' => $model->fk_tool_id]) : $model->fkTool->tool_name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
            		'attribute' => 'fk_tool_id',
            		'data' => ArrayHelper::map(app\models\Tool::find()->asArray()->all(), 'id', 'tool_name'),
            		'options' => ['placeholder' => Yii::t('app', 'Select ...'), 'id' =>'select2_fkTool', 'multiple' => true],
            		'pluginOptions' => [
            				'allowClear' => true
            		],
			]),
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkTool === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
            [
             'label' => Yii::t('app', 'Data Delivery Type'),
             'value' => function($model) {
             		return $model->fk_data_delivery_type_id == "" ? $model->fk_data_delivery_type_id : (isset($_GET["searchShow"]) ? $model->fkDataDeliveryType->name . ' [' . $model->fk_data_delivery_type_id . ']' : ($model->fkDataDeliveryType=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkDataDeliveryType', 'this_id' => $model->fk_data_delivery_type_id]) : $model->fkDataDeliveryType->name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
            		'attribute' => 'fk_data_delivery_type_id',
            		'data' => ArrayHelper::map(app\models\DataDeliveryType::find()->asArray()->all(), 'id', 'name'),
            		'options' => ['placeholder' => Yii::t('app', 'Select ...'), 'id' =>'select2_fkDataDeliveryType', 'multiple' => true],
            		'pluginOptions' => [
            				'allowClear' => true
            		],
			]),
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkDataDeliveryType === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
/*            [
             'label' => Yii::t('app', 'Contact Group As Data Owner'),
             'value' => function($model) {
             		return $model->fk_contact_group_id_as_data_owner == "" ? $model->fk_contact_group_id_as_data_owner : (isset($_GET["searchShow"]) ? $model->fkContactGroupIdAsDataOwner->name . ' [' . $model->fk_contact_group_id_as_data_owner . ']' : ($model->fkContactGroupIdAsDataOwner=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkContactGroupIdAsDataOwner', 'this_id' => $model->fk_contact_group_id_as_data_owner]) : $model->fkContactGroupIdAsDataOwner->name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
            		'attribute' => 'fk_contact_group_id_as_data_owner',
            		'data' => ArrayHelper::map(app\models\ContactGroup::find()->asArray()->all(), 'id', 'name'),
            		'options' => ['placeholder' => Yii::t('app', 'Select ...'), 'id' =>'select2_fkContactGroupIdAsDataOwner', 'multiple' => true],
            		'pluginOptions' => [
            				'allowClear' => true
            		],
			]),
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkContactGroupIdAsDataOwner === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
*/        ],
    ]); ?>
