...
<!--  	// auto commented via gii/CRUD    <?= $form->field($model, 'fk_object_type_id')->textInput() ?>  -->
	...
	<?php
		...
		Pjax::begin(['id'=>'id-pjax-contact-group-form_fk_client_id', 'timeout' => false, 'enablePushState' => false]);
		echo $form->field($model, 'fk_client_id', ['template' => '<label class="control-label">{label}</label><div class="input-group">{input}<span class="input-group-btn">'.Html::button('+', [
			'value' => Url::to('index.php?r=client/create&isfrommodal=true&modalparent=id-pjax-contact-group-form_fk_client_id&refreshfield=name__fk_client_id')
