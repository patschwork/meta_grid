...
?>
	...
	?>
	    <?= GridView::widget([
		'tableOptions' => ['id' => 'grid-view-sourcesystem', 'class' => 'table table-striped table-bordered'],
        ...
        'columns' => [
            ...
            [
             ...
             'value' => function($model) {
             		...
             		'filter' => Select2::widget([
             				...
             				'data' => ArrayHelper::map(VSourcesystemSearchinterface::find()->select(['fk_client_id', 'client_name'])->distinct()->asArray()->all(), 'fk_client_id', 'client_name'),
            ...
            'filter' => Select2::widget([
            		...
            		'data' => ArrayHelper::map(VSourcesystemSearchinterface::find()->select(['fk_project_id', 'project_name'])->distinct()->asArray()->all(), 'fk_project_id', 'project_name'),
            ...
            [
             ...
             'value' => function($model) {
             		return $model->fk_contact_group_id_as_supporter == "" ? $model->fk_contact_group_id_as_supporter : (isset($_GET["searchShow"]) ? $model->fkContactGroupIdAsSupporter->name . ' [' . $model->fk_contact_group_id_as_supporter . ']' : ($model->fkContactGroupIdAsSupporter=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkContactGroupIdAsSupporter', 'this_id' => $model->fk_contact_group_id_as_supporter]) : $model->fkContactGroupIdAsSupporter->name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
			...
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkContactGroupIdAsSupporter === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
        ],
    ]); ?>
	...
	if ($Utils->get_app_config("floatthead_for_gridviews") == 1)
	{
		\bluezed\floatThead\FloatThead::widget(
			[
				'tableId' => 'grid-view-sourcesystem', 
				'options' => [
					'top'=>'50'
				]
			]
