...
use vendor\meta_grid\contactgroup_select\ContactGroupSelectWidget;use yii\widgets\Pjax;
...
<!--  	// auto commented via gii/CRUD    <?= $form->field($model, 'fk_object_type_id')->textInput() ?>  -->
	...
	<?php
		...
		Pjax::begin(['id'=>'id-pjax-sourcesystem-form_fk_project_id', 'timeout' => false, 'enablePushState' => false]);
		echo $form->field($model, 'fk_project_id', ['template' => '<label class="control-label">{label}</label><div class="input-group">{input}<span class="input-group-btn">'.Html::button('+', [
			'value' => Url::to('index.php?r=project/create&isfrommodal=true&modalparent=id-pjax-sourcesystem-form_fk_project_id&refreshfield=name__fk_project_id')
 ...
 <!--  	// auto commented via gii/CRUD    <?= $form->field($model, 'description') ?>  -->
	...
	<?php
		...
		Pjax::begin(['id'=>'id-pjax-sourcesystem-form_fk_contact_group_id_as_supporter', 'timeout' => false, 'enablePushState' => false]);
		echo $form->field($model, 'fk_contact_group_id_as_supporter', ['template' => '<label class="control-label">{label}</label><div class="input-group">{input}<span class="input-group-btn">'.Html::button('+', [
			'value' => Url::to('index.php?r=contactgroup/create&isfrommodal=true&modalparent=id-pjax-sourcesystem-form_fk_contact_group_id_as_supporter&refreshfield=sourcesystem-fk_contact_group_id_as_supporter')
			, 'class' => 'btn btn-primary'
			, 'id' => 'modalButtonCreate__fk_contact_group_id_as_supporter'
		...
		Pjax::end();
 	?>
 <!--  	// auto commented via gii/CRUD    <?= $form->field($model, 'fk_contact_group_id_as_supporter')->textInput() ?>  -->
...
$(function(){
	...
	$('#modalButtonCreate__fk_contact_group_id_as_supporter').click(function() {
		$('#modalCreate').modal('show')
		.find('#modalContent')
		.load($(this).attr('value'));
	});
