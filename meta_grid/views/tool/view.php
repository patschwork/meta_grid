...
<div class="tool-view">
    ...
    <p>
        <?= Yii::$app->user->identity->isAdmin || Yii::$app->User->can('create-tool')  ? Html::a(Yii::t('app', 'Update'), ['update', 'id' => $model->id], ['class' => 'btn btn-primary']) : "" ?>

        <?= Yii::$app->user->identity->isAdmin || Yii::$app->User->can('delete-tool')  ? Html::a(Yii::t('app', 'Delete'), ['delete', 'id' => $model->id], [
    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
             ...
             'value' =>         	    $model->fk_tool_type_id == "" ? $model->fk_tool_type_id : ($model->fkToolType === NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkToolType', 'this_id' => $model->fk_tool_type_id]) : $model->fkToolType->name)
            ],
            'tool_name:ntext',
            'vendor:ntext',
            'version:ntext',
