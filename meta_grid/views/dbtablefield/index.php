	...
	?>
        ...
        'columns' => [
            ...
            'filter' => Select2::widget([
            		'model' => $searchModel,
            		'attribute' => 'schemaInfoFromLocation',
            		'data' => ArrayHelper::map(app\models\VDbTableFieldSearchinterface::find()->asArray()->all(), 'schemaInfoFromLocation', 'schemaInfoFromLocation'),
            ...
            [
             ...
             'value' => function($model) {
             		return $model->fk_db_table_id == "" ? $model->fk_db_table_id : (isset($_GET["searchShow"]) ? $model->fkDbTable->name . ' [' . $model->fk_db_table_id . ']' : ($model->fkDbTable=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkDbTable', 'this_id' => $model->fk_db_table_id]) : $model->fkDbTable->name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
			...
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkDbTable === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
            'datatype:ntext',
