	...
	?>
        ...
        'columns' => [
            ...
            [
             ...
             'value' => function($model) {
             		return $model->fk_data_transfer_type_id == "" ? $model->fk_data_transfer_type_id : (isset($_GET["searchShow"]) ? $model->fkDataTransferType->name . ' [' . $model->fk_data_transfer_type_id . ']' : ($model->fkDataTransferType=== NULL ? Yii::t('app', "Can't lookup the {relFieldname} name (for id {this_id})", ['relFieldname' => 'fkDataTransferType', 'this_id' => $model->fk_data_transfer_type_id]) : $model->fkDataTransferType->name));
             		},
            'filter' => Select2::widget([
            		'model' => $searchModel,
			...
			'contentOptions' => function ($model, $key, $index, $column) {
			     return $model->fkDataTransferType === NULL ? ['style' => 'color: red'] : [];
			 },
            ],
