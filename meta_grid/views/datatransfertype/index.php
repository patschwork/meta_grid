...
?>
    ...
    <p>
		<?= Yii::$app->user->identity->isAdmin || Yii::$app->User->can('create-datatransfertype')  ? Html::a(
		Yii::t('app', 'Create {modelClass}', ['modelClass' => Yii::t('app', 'Data Transfer Type'),]), ['create'], ['class' => 'btn btn-success']) : "" ?>
					</p>
	...
	?>
	    <?= GridView::widget([
		'tableOptions' => ['id' => 'grid-view-data-transfer-type', 'class' => 'table table-striped table-bordered'],
	...
	<?php 	if (\vendor\meta_grid\helper\Utils::get_app_config("floatthead_for_gridviews") == 1)
	{
		\bluezed\floatThead\FloatThead::widget(
			[
				'tableId' => 'grid-view-data-transfer-type', 
				'options' => [
					'top'=>'50'
				]
			]
