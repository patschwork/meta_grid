	...
	if (substr($column->name,0,3)=="fk_")
		...
		$genCode = "";	   
			...
			$genCode .= "            [\n";
    			...
    			$genCode .= "             'value' => ";
				$genCode .= "             		    \$model->".$column->name." == \"\" ? \$model->".$column->name." : (\$model->".$relFieldname."->fkClient === NULL ? Yii::t('app', \"Can't lookup the client name (for project {fk_project_id})\", ['fk_project_id' => \$model->".$column->name."]) : \$model->".$relFieldname."->fkClient->name)\n";
    			$genCode .= "            ],\n"; 		
    		}
    		...
    		$genCode .= "             'value' => ";
            $genCode .= "        	    \$model->".$column->name." == \"\" ? \$model->".$column->name." : (\$model->".$relFieldname." === NULL ? Yii::t('app', \"Can't lookup the {relFieldname} name (for id {this_id})\", ['relFieldname' => '$relFieldname', 'this_id' => \$model->".$column->name."]) : \$model->".$relFieldname."->".$nameFieldProperty.")\n";
    		$genCode .= "            ],\n";
