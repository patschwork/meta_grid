...
//         	}        	
        	...
        	if ($column->name=="fk_project_id")
        	{
				...
				$genCode .= "             		return \$model->".$column->name." == \"\" ? \$model->".$column->name." : (\$model->".$relFieldname."->fkClient === NULL ? Yii::t('app', \"Can't lookup the client name (for project {fk_project_id})\", ['fk_project_id' => \$model->".$column->name."]) : \$model->".$relFieldname."->fkClient->name);\n";
	        	$genCode .= "             		},\n";
				...
				// if no lookup possible, show red text
				$genCode .= "			'contentOptions' => function (\$model, \$key, \$index, \$column) {" . "\n";
				$genCode .= "			     return \$model->".$relFieldname."->fkClient === NULL ? ['style' => 'color: red'] : [];" . "\n";
				$genCode .= "			 }," . "\n";
        	...
        	$genCode .= "             'value' => function(\$model) {\n";
        	$genCode .= "             		return \$model->".$column->name." == \"\" ? \$model->".$column->name." : ".'(isset($_GET["searchShow"]) ? '."\$model->".$relFieldname."->$nameFieldProperty . ' [' . \$model->".$column->name .  " . ']'" . " : " . "(\$model->".$relFieldname."=== NULL ? Yii::t('app', \"Can't lookup the {relFieldname} name (for id {this_id})\", ['relFieldname' => '$relFieldname', 'this_id' => \$model->".$column->name."]) : \$model->".$relFieldname."->".$nameFieldProperty."))";
			$genCode .= ";\n";
        	$genCode .= "             		},\n";
			...
			// if no lookup possible, show red text
			$genCode .= "			'contentOptions' => function (\$model, \$key, \$index, \$column) {" . "\n";
			$genCode .= "			     return \$model->$relFieldname === NULL ? ['style' => 'color: red'] : [];" . "\n";
			$genCode .= "			 }," . "\n";
