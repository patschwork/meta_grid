CREATE OR REPLACE FUNCTION public.trig_func_log()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
   ...
   IF TG_OP='UPDATE' THEN
    ...
    RAISE NOTICE 'execute sql A: %', upd_uuid_for_tmptable_stmt; 
   END IF;
   tmp_table_fields := (SELECT "columns" FROM v_fk_columns_for_triggers WHERE table_name=tmp_table);
   insert_into_log_stmt := format('INSERT INTO %I (%s) SELECT %s FROM %I', TG_TABLE_NAME||'_log', tmp_table_fields, tmp_table_fields, tmp_table);
